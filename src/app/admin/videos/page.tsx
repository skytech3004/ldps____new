...
import { X, ChevronLeft, ChevronRight, Film, Trash2, Upload, Plus } from "lucide-react";
...
export default function AdminVideosPage() {
  ...
  const handlePrevPreview = (e?: React.MouseEvent) => {
    ...
    const currentIdx = videoItems.findIndex((item) => item._id === activePreview);
    const newIdx = currentIdx === 0 ? videoItems.length - 1 : currentIdx - 1;
    const nextId = videoItems[newIdx]._id;
  ...
  const handleNextPreview = (e?: React.MouseEvent) => {
    ...
    if (activePreview === null) return;
    const currentIdx = videoItems.findIndex((item) => item._id === activePreview);
    const newIdx = currentIdx === videoItems.length - 1 ? 0 : currentIdx + 1;
    const nextId = videoItems[newIdx]._id;
    if (nextId) setActivePreview(nextId);
  };
  ...
  const handleDeleteItem = async (id: string) => {
    if (!id) return;
    const confirmed = window.confirm("Are you sure you want to delete this video highlight?");
    if (!confirmed) return;
  ...
  const previewItem = videoItems.find((item) => item._id === activePreview);
      ...
      <div className="mb-8 flex justify-between items-start md:items-center flex-col md:flex-row gap-4">
        <div>
          ...
          <p className="text-white/60 text-xs mt-2">
            Upload and manage videos displayed under 'Campus Video Highlights' on the homepage.
          </p>
        </div>
        <button
        ...
        <>
                ...
                {inputMode === "url" ? (
                  ...
                  <label className="relative px-4 py-2 border-2 border-dashed border-white/10 rounded-lg cursor-pointer hover:border-accent transition-colors flex items-center justify-center bg-[#081736]">
                    ...
                    <span className="text-white/60 font-semibold">
                      {uploading ? "Uploading..." : formData.src ? "✓ Video Uploaded" : "Click to Upload Video"}
                    </span>
                  </label>
                )}
              </div>
          ...
          {videoItems.length === 0 && (
            <div className="text-center py-12 text-slate-500">
              <p className="text-lg">No video highlights yet. Add one to get started!</p>
            </div>
          )}
                ...
                >
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] md:text-xs font-bold text-slate-400 uppercase">
                      Video {videoItems.findIndex((item) => item._id === activePreview) + 1} of {videoItems.length}
                    </p>
                  </div>
