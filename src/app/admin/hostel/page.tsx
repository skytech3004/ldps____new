...
export default function AdminHostelPage() {
  const [activeTab, setActiveTab] = useState<"facilities" | "fees" | "rules">("facilities");
  ...
  async function handleImageUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      formData.append("title", `Facility - ${facName || "New Item"}`);
  ...
  async function handleFormSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/hostel?type=${activeTab}`, {
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="flex border-b border-white/10">
        {(["facilities", "fees", "rules"] as const).map((tab) => (
          <button
            key={tab}
          ...
          >
            {tab === "facilities" ? "Facilities Grid" : tab === "fees" ? "Fee Structure" : "Rules & Policies"}
          </button>
        ))}
      </div>
