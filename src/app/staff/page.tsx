...
function countSupportStaff(roster: RosterMember[]) {
  return roster.filter((member) =>
    /office|librarian|front desk|peon|sweeper|supdt|lab asst/i.test(member.designation)
  ).length;
}
      ...
      <section className="relative pt-36 pb-12 md:pt-44 md:pb-16 px-6 bg-gradient-to-br from-primary to-[#2c246b] text-white">
        ...
        <div className="max-w-7xl mx-auto relative z-10 space-y-3">
          ...
          <p className="text-white/60 font-medium text-xs md:text-sm max-w-2xl">
            Meet our dedicated team of educators, administrators, and leadership committed to academic excellence.
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {roster.map((member, idx) => (
              <article
                ...
                className="bg-white border border-primary/10 rounded-[2rem] p-6 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col items-center text-center group relative overflow-hidden"
              >
                {/* Order Index Badge (1, 2, 3...) */}
                <div className="absolute top-4 left-4 z-10 w-7 h-7 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-primary font-mono font-bold text-xs shadow-sm">
                  #{idx + 1}
                </div>
