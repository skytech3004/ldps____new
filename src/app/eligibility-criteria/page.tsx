...
import { Phone, ClipboardList, CheckCircle, ArrowRight, ShieldAlert } from "lucide-react";
...
export default function EligibilityCriteria() {
  ...
  useEffect(() => {
    async function fetchDbPage() {
      try {
        const res = await fetch("/api/admin/pages?slug=eligibility-criteria");
        if (res.ok) {
  ...
  return (
    <PageLayout
      groupName="Academics"
      pageTitle={dbContent?.title || "Admission Guidelines"}
      subtitle={dbContent?.subtitle || "Complete details on guidelines, requirements, procedures, and required documents."}
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
        ...
        <div className="lg:col-span-6 space-y-8">
          <PageSectionHeader
            ...
            subtitle="Transparent and merit-based admission policies."
            centered={false}
          />
